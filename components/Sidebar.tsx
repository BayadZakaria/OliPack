...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, setActiveSection, user, onLogout, onShowAuth, forceMobile = false }) => {
  ...
  useEffect(() => {
    if (user?.role === 'ADMIN' || user?.role === 'VENDEUR') {
      const checkNotifications = async () => {
        // Logique de notification simplifiée pour les ventes
        ...
        const pending = [...shopOrders, ...studioOrders].filter(o => 
          (user?.role === 'ADMIN' ? true : o.ville === user?.ville) && o.status === 'EN_ATTENTE'
        ).length;
        setTotalNotifications(pending);
      };
      checkNotifications();
  ...
  const canSee = (section: AppSection) => {
    ...
    if (role === 'VENDEUR') {
      // Fix TypeScript error by casting the array to allow comparison with section: AppSection
      return ([AppSection.HOME, AppSection.SALES_CONTROL, AppSection.IMPACT, AppSection.PROFILE] as any[]).includes(section);
    }

    if (([AppSection.HOME, AppSection.PRODUCTS, AppSection.ATELIER, AppSection.STUDIO, AppSection.IMPACT] as any[]).includes(section)) {
        if (role === 'TECHNICIEN' && section !== AppSection.IMPACT) return section === AppSection.HOME; 
        return true;
    }
    ...
    switch (role) {
      ...
      default: return ([AppSection.HOME, AppSection.PRODUCTS, AppSection.ATELIER, AppSection.STUDIO, AppSection.IMPACT] as any[]).includes(section);
    }
  };
  ...
  const allNavItems = [
    ...
    { id: AppSection.SALES_CONTROL, icon: ClipboardList, label: 'Commandes', badge: (role === 'VENDEUR' || role === 'ADMIN') ? totalNotifications : null },
    { id: AppSection.ATELIER, icon: Info, label: 'Infos' },
    { id: AppSection.COLLECTION, icon: Truck, label: 'Collecte' },
    { id: AppSection.ADMIN_CONTROL, icon: ShieldAlert, label: 'Supervision' },
