...
const Products: React.FC<ProductsProps> = ({ user, onRequireAuth }) => {
  ...
  const products = [
    ...
    { id: 5, name: "Vase Intelligent 'Flora'", category: "Objets Connectés", price: "249.00", unit: "DH/unité", desc: "Vase en PHA bioplastique avec puce NFC intégrée et QR Code. Permet de suivre la santé de votre plante via l'App OliPack (à venir).", img: "https://images.unsplash.com/photo-1572297821074-3fd50b09bfc6?q=80&w=400&auto=format&fit=crop", eco: 100, isSmart: true, tech: ["NFC Inside", "QR Tracking"] },
    { id: 6, name: "Pochette Bio-NFC", category: "Objets Connectés", price: "120.00", unit: "DH/unité", desc: "Pochette protectrice en matière souple OliPack. Échangez vos réseaux sociaux par simple contact (NFC) avec d'autres utilisateurs.", img: "https://images.unsplash.com/photo-1603539947678-cd3954ed515d?q=80&w=400&auto=format&fit=crop", eco: 100, isSmart: true, tech: ["Social NFC"] },
    { id: 3, name: "Biopolymère PHA", category: "Innovation", price: "0.00", unit: "Sur Devis", desc: "Plastique 100% biodégradable produit par biosynthèse à partir de margines purifiées.", img: "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=400&auto=format&fit=crop", eco: 100, stock: "Laboratoire R&D" },
    { id: 4, name: "Engrais Bio-Organique", category: "Agriculture", price: "0.80", unit: "DH/kg", desc: "Fertilisant riche en potassium et matières organiques, stabilisé après traitement.", img: "https://images.unsplash.com/photo-1585314062340-f1a5a7c9328d?q=80&w=400&auto=format&fit=crop", eco: 100, stock: "Marrakech" }
  ];
  ...
  const handlePostReview = async (productId: number) => {
    if (!user) {
      onRequireAuth();
      return;
    }
    ...
    <div className="space-y-8 animate-in pb-24 max-w-6xl mx-auto relative">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-slate-900 flex items-center gap-3 tracking-tight">
            <ShoppingBag className="text-emerald-600 w-10 h-10" /> Boutique <span className="text-emerald-600 italic">Connectée</span>
          </h1>
          <p className="text-slate-500 font-medium">L'Or Vert se digitalise : découvrez nos objets intelligents en PHA.</p>
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
        {['Tous', 'Objets Connectés', 'Innovation', 'Énergie', 'Agriculture'].map(cat => (
          <button key={cat} onClick={() => setFilter(cat)} className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all shrink-0 border ${filter === cat ? 'bg-slate-900 text-white border-slate-900 shadow-lg' : 'bg-white text-slate-500 border-slate-200'}`}>{cat}</button>
        ))}
      </div>
              ...
              <div className="p-8 flex flex-col flex-1 space-y-4">
                 <div className="space-y-1">
                   <div className="flex items-center justify-between">
                      ...
                      {p.isSmart && <Smartphone className="text-emerald-400 w-5 h-5" />}
                   </div>
                   <p className="text-xs text-slate-500 leading-relaxed line-clamp-2">{p.desc}</p>
                 </div>

                 {/* Tech Features if smart */}
                 {p.isSmart && p.tech && (
                    <div className="flex flex-wrap gap-2">
                       {p.tech.map(t => (
                          <span key={t} className="bg-emerald-50 text-emerald-700 text-[8px] font-black px-2 py-1 rounded-lg uppercase border border-emerald-100 flex items-center gap-1">
                             <QrCode className="w-2.5 h-2.5" /> {t}
                          </span>
                       ))}
                    </div>
                 )}
                 ...
                 <div className="bg-slate-50 rounded-2xl p-4 space-y-3">
                    <button 
                      onClick={() => setActiveReviewId(activeReviewId === p.id ? null : p.id)}
                      className="flex items-center gap-2 text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:underline"
                    >
                       <MessageSquare className="w-3 h-3" /> {activeReviewId === p.id ? 'Fermer les avis' : `Voir les ${productReviews.length} avis`}
                    </button>
                       ...
                       <div className="space-y-4 animate-in slide-in-from-top-2">
                          <div className="max-h-40 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                             {productReviews.length === 0 ? (
                                <p className="text-[9px] text-slate-400 italic">Aucun avis pour le moment.</p>
                             ) : (
                                productReviews.map(r => (
                                   <div key={r.id} className="bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm">
                                      <div className="flex justify-between items-center mb-1">
                                         <span className="text-[9px] font-black text-slate-800">{r.userName}</span>
                                         <Star className="w-2.5 h-2.5 text-amber-400 fill-amber-400" />
                                      </div>
                                      <p className="text-[9px] text-slate-500 font-medium leading-tight">{r.comment}</p>
                                   </div>
                                ))
                             )}
                          </div>
           ...
           <aside className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right-10 duration-500">
              <header className="p-8 border-b border-slate-100 flex items-center justify-between">
                 <div className="flex items-center gap-4">
                    ...
                    <div><h2 className="text-xl font-black text-slate-900 tracking-tight">Mon Panier</h2><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{cart.length} article(s)</p></div>
                 </div>
              ...
              {cart.length > 0 && (
                <footer className="p-8 bg-slate-50 border-t border-slate-100 space-y-6">
                   <div className="flex justify-between items-end"><span className="text-sm font-black text-slate-900 uppercase tracking-tighter">Total à régler</span><span className="text-3xl font-black text-emerald-600 tracking-tighter">{cartTotal.toFixed(2)} DH</span></div>
                   <button onClick={() => { setOrderSuccess(true); setTimeout(() => { setCart([]); setShowCart(false); setOrderSuccess(false); }, 3000); }} className="w-full py-5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-sm shadow-xl flex items-center justify-center gap-3">
                      {orderSuccess ? <PackageCheck className="w-6 h-6" /> : <CreditCard className="w-6 h-6" />} {orderSuccess ? 'COMMANDE RÉUSSIE !' : 'VALIDER MA COMMANDE'}
                   </button>
                </footer>
              )}
           </aside>
